...
const classeSubtitle = computed(() =>
  (!classeOpen.value && isClasseComplete.value) ? `${classe.value.value}, ${voie.value.value}` : undefined
)
const rightNote = computed(() => isClasseComplete.value ? '' : 'À compléter')

const minWidthFrom = (arr: string[], base = 8) => {
  const longest = (arr || []).reduce((a, b) => (a.length > b.length ? a : b), '')
  return `${Math.max(longest.length + 2, base)}ch`
}
const classeMinWidth = computed(() => minWidthFrom(classe.value?.options || [], 8))
const voieMinWidth = computed(() => minWidthFrom(voie.value?.options || [], 10))
...
function classeConfirmed() {
  if (!isClasseComplete.value) return
...
function onClasseChange(next: { classe?: string; voie?: string }) {
  if (typeof next.classe === 'string') classe.value = { ...classe.value, value: next.classe }
  if (typeof next.voie === 'string') voie.value = { ...voie.value, value: next.voie }
}

function goToResult() {
  if (!isClasseComplete.value) return
  router.push({ path: '/result', query: { lycee: lycee.value, city: city.value } })
}
</script>
...
<template>
  <main class="mx-auto max-w-[420px] md:max-w-[800px] space-y-4 pb-16">
    ...
    <div class="rounded-2xl bg-white p-0">
      <CollapsibleCard
        :title="'Classe'"
        :subtitle="classeSubtitle"
        :rightNote="rightNote"
        :open="classeOpen"
        :onConfirm="classeConfirmed"
        :confirmDisabled="!isClasseComplete"
      >
        <div class="px-4 pt-2 pb-4">
          <div class="mt-2 flex flex-wrap gap-2">
            <button
              v-for="opt in classe.options"
              :key="opt"
              class="shrink-0 rounded-full px-4 py-2 text-sm transition whitespace-nowrap"
              :style="{ minWidth: classeMinWidth }"
              :class="opt===classe.value ? 'border border-black bg-white text-black' : 'border-transparent bg-gray-100 hover:bg-gray-200 text-gray-800'"
              @click="onClasseChange({ classe: opt })"
            >
              {{ opt }}
            </button>
          </div>
          ...
          <div class="mt-2 flex flex-wrap gap-2">
            <button
              v-for="opt in voie.options"
              :key="opt"
              class="shrink-0 rounded-full px-4 py-2 text-sm transition whitespace-nowrap"
              :style="{ minWidth: voieMinWidth }"
              :class="opt===voie.value ? 'border border-black bg-white text-black' : 'border-transparent bg-gray-100 hover:bg-gray-200 text-gray-800'"
              @click="onClasseChange({ voie: opt })"
            >
              {{ opt }}
            </button>
          </div>
