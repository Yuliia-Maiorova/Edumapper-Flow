...
const props = defineProps<{ modelValue: boolean; allSchools: readonly string[]; selected: string }>()
...
const filtered = computed(() => {
  ...
  if (!q) return props.allSchools
  return props.allSchools.filter(l => l.toLowerCase().includes(q))
})
    ...
    <div v-if="open" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-4">
      <div class="w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl bg-white">
        <div class="px-4 py-3 border-b border-gray-200">
          <h4 class="text-[15px] font-semibold">Change school</h4>
        </div>
        <div class="p-4">
          <input type="text" placeholder="Search a school" class="w-full rounded-full border border-gray-300 px-4 py-2 text-[14px] placeholder:text-gray-400" v-model="query" />
          <div class="mt-3 max-h-64 overflow-auto rounded-xl border border-gray-200 p-3">
            <label v-for="l in filtered" :key="l" class="mb-2 inline-flex items-center last:mb-0">
              <input class="peer sr-only" type="radio" name="school" :value="l" v-model="local">
        ...
        <div class="flex justify-end gap-2 px-4 pb-4">
          <button class="rounded-full border border-gray-300 px-4 py-2 text-[13px]" @click="open=false">Cancel</button>
          <button class="rounded-full bg-black px-4 py-2 text-[13px] text-white" @click="onConfirm">Confirm</button>
        </div>
      </div>
    </div>
  </transition>
